...
import harbour.noto.FolderListModel 1.0
...
Page {
    ...
    function openFile(path) {
        if (_fm.isFile(path)) {
            ...
            if(mimeinfo[0] === "text")
            {
                ...
                mainWindow.infoBanner.showText(qsTr("Unknown file format."));
    ...
    SilicaListView {
        ...
        width: parent.width
        height: {
            if (savePanel.open) parent.height - (savePanel.height + Theme.paddingMedium)
            else parent.height
        }
        clip: true
        ...
        delegate: BackgroundItem {
            ...
            ListItem {
                ...
                onClicked: {
                    ...
                    if (fileIsDir) {
                        var anotherFM = pageStack.push(Qt.resolvedUrl("OpenDialog.qml"), {"path": filePath, "dataContainer": dataContainer, "selectMode": selectMode, "multiSelect": multiSelect, "saveMode": saveMode});
    ...
    FancyScroller {
        ...
        onUpScrolling: if (saveMode && !savePanel.open) savePanel.show()
    }
    ...
    DockedPanel {
        ...
        Item {
            ...
            TextField {
                id: savefilename
                width: parent.width - (saveBtn.width + ( 3* Theme.paddingMedium ))
                anchors.left: parent.left
                anchors.leftMargin: Theme.paddingMedium
                placeholderText: qsTr("Filename")
                label: qsTr("Filename")
                text: {
                    if (dataContainer._noteTitle != "") dataContainer._noteTitle + ".txt"
                }
            }
            Button {
                id: saveBtn
                ...
                text: qsTr("Save")
                onClicked: {
                    if (savefilename.text != "") _fileio.write(path + "/" + savefilename.text, dataContainer._noteText);
                    pageStack.pop(dataContainer);
                }
            }
        }
